...
GLOBAL_CACHE = {
    ...
    "NIKKEI_FUT": {"symbol": "NIKKEI_FUT", "ltp": 0.0, "market_status": "RED"},
    "WTI_CRUDE": {"symbol": "WTI_CRUDE", "ltp": 0.0, "ch": 0.0, "chp": 0.0, "market_status": "RED"},
...
def fetch_tradingview_global(session, api_timeout, load_cache_func, save_cache_func, health_ok_func, health_fail_func):
    ...
    payload = {
        "symbols": {
            "tickers": [
                "TVC:DJI", "CBOT:YM1!", "CBOT:YM", "TVC:IXIC", "TVC:SPX", "SP:SPX", "AMEX:SPY", 
                "TVC:NI225", "OSE:NK2!", "NYMEX:CL1!", "NYMEX:BZ1!", "TVC:GOLD"
            ]
        },
                ...
                for row in data.get("data", []):
                    ...
                    if len(vals) >= 3:
                        ...
                        if "DJI" in s: 
                            ...
                            GLOBAL_CACHE["NIKKEI_FUT"] = {"symbol": "NIKKEI_FUT", "ltp": round(p, 2), "market_status": "RED"}
                        elif "CL1!" in s: 
                            GLOBAL_CACHE["WTI_CRUDE"] = {**item_data, "symbol": "WTI_CRUDE", "market_status": "RED"}
                        elif "BZ1!" in s: 
...
def get_global(symbol: str):
    ...
    if sym == "NIKKEI":
        nikkei_data = dict(GLOBAL_CACHE.get("NIKKEI", {"symbol": "NIKKEI", "ltp": 0, "ch": 0, "chp": 0, "market_status": "RED"}))
        fut_item = GLOBAL_CACHE.get("NIKKEI_FUT", {})
        fut_val = fut_item.get("ltp", 0) if isinstance(fut_item, dict) else fut_item
        if fut_val > 0:
            nikkei_data["fut"] = fut_val
        return nikkei_data
